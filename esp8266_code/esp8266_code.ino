...
//Define macros
#define HTTP_OTA_ADDRESS      F("172.16.53.131")       //TODO arrange this as configurable info //Address of OTA update server
...
// General variables
float deep_sleep_time = 10000000;
...
void reconnect() {
  ...
  while (!client.connected() && tries < 3) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.subscribe("GRUPOG/DeepSleep", 1); 
      client.loop();
...
void callback(String &topic, String &payload) {
  ...
  // TODO: Arreglar esto
   String param_DeepSleep = payload;
  if (topic == "GRUPOG/DeepSleep"){
    if (param_DeepSleep.toFloat() >= 1){ //The minimum time for DeepSleep is 1min
      deep_sleep_time  = param_DeepSleep.toFloat() * 5000000;
    }
    else {
      deep_sleep_time  = 1000000; //The default value if the msg does not give a valid number is 1min
    }
  }
}
